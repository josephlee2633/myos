...
/* ADC ADC_OVSCR mask */
#define OVSCR_DRES_RESET                ((uint32_t)0xFFFFCFFF)
#define OVSCR_OVSS_RESET                ((uint32_t)0xFFFFFE1F)
#define OVSCR_OVSR_RESET                ((uint32_t)0xFFFFFFE3)
#define OVSCR_TOVS_SET                  ((uint32_t)0x00000200)
#define OVSCR_OVSE_SET                  ((uint32_t)0x00000001)
...
/**
  ...
  *   This parameter can be as follows:
  *     @arg ADC_EXTERNAL_TRIG_INSERTCONV_T1_TRGO: Timer1 TRGO event (used in ADC1, ADC2 and ADC3)
  *     @arg ADC_EXTERNAL_TRIG_INSERTCONV_T1_CC4: Timer1 capture compare4 (used in ADC1, ADC2 and ADC3)
  *     @arg ADC_EXTERNAL_TRIG_INSERTCONV_T2_TRGO: Timer2 TRGO event (used in ADC1 and ADC2)
  ...
  *     @arg ADC_EXTERNAL_TRIG_INSERTCONV_T4_TRGO: Timer4 TRGO event (used in ADC1 and ADC2)
  ...
  *     @arg ADC_EXTERNAL_TRIG_INSERTCONV_T5_TRGO: Timer5 TRGO event (used in ADC3)
...
void ADC_ClearIntBitState(ADC_TypeDef* ADCx, uint16_t ADC_INT)
{
    uint8_t temp = 0;
    /* Get the ADC interrupt mask index */
    temp = (uint8_t)(ADC_INT >> 8);
    /* Clear ADCx interrupt pending bits */
    ADCx->STR = ~(uint32_t)temp;
}
...
/**
  * @brief  Enable or disable ADCx's Oversampler.Trigger.
  * @param  ADCx: the ADC interface where x can be 1..3.
  * @param  NewValue: the state of the Oversampler.Trigger.
  *   This parameter can be: ENABLE or DISABLE.
  * @retval None
  */
void ADC_TriggeredOversampling_Enable(ADC_TypeDef* ADCx,TypeState NewValue)
{
    if (NewValue != DISABLE)
    {
        /* Enable the ADCx's Oversampler.Trigger.*/
        ADCx->OVSCR |= OVSCR_TOVS_SET;
    }
    else
    {
        /* Disable the ADCx's Oversampler.Trigger.*/
        ADCx->OVSCR &= ~OVSCR_TOVS_SET;
    }
}

/**
  * @brief  Enable or disable the ADCx's Oversampler.
  * @param  ADCx: the ADC interface where x can be 1..3.
  * @param  NewValue: the state of the Oversampler.
  *   This parameter can be: ENABLE or DISABLE.
  * @retval None
  */
void ADC_Oversampler_Enable(ADC_TypeDef* ADCx,TypeState NewValue)
{
    if (NewValue != DISABLE)
    {
        /* Enable the ADCx's Oversampler.*/
        ADCx->OVSCR |= OVSCR_OVSE_SET;
    }
    else
    {
        /* Disable the ADCx's Oversampler.*/
        ADCx->OVSCR &= ~OVSCR_OVSE_SET;
    }
}
