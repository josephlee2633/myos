...
void ETH_MDIOInit(void) 
{
    ...
    /* ((hclk >= 90000000)&&(hclk <= 120000000)) */    
    else 
    {
        /* Clock Range between 90-120 MHz */   
...
/**
  ...
  *   Select one of the follwing values :
  *     @arg ETH_MAC_FLAG_TMST  : Time stamp trigger flag   
...
/**
  ...
  *   Select one of the follwing values :
  *     @arg ETH_MAC_INT_TMST  : Time stamp trigger interrupt  
...
void ETH_DMARxDescRingModeInit(ETH_DMADESCTypeDef *DMARxDescTab, uint8_t *pRxBuff1, uint8_t *pRxBuff2, uint32_t RxBuffCnt)
{
    ...
    for(num=0; num < RxBuffCnt; num++)
    {
        ...
        if(num == (RxBuffCnt-1))
        {
            /* Set ETH_DMARXDESC_RERM bit as Receive End */
            DMARxDesc->ControlBufferSize |= ETH_DMARXDESC_RERM;
        }
    }
                                   ...
                                   uint8_t* pTxBuff, uint32_t TxBuffCnt)
{
    ...
    for(num=0; num < TxBuffCnt; num++)
    {
        /* Get the pointer to the next descriptor of the Tx Desc table */
        DMATxDesc = DMATxDescTab+num;
        /* Set TCHM bit and enable PTP with desc status */
...
uint32_t ETH_HandlePTPTxPkt(uint8_t *pbuf, uint16_t size, uint32_t *PTPTxTab)
{
    ...
    {
        timeout++;
    } while (!(DMACurrentTxDesc->Status & ETH_DMATXDESC_TTMSS) && (timeout < PHY_READ_TO));
    /* Return ERROR due to timeout */
    if(timeout == PHY_READ_TO)
    {
        return ERROR;
    ...
    *PTPTxTab = DMACurrentTxDesc->Buffer2NextDescAddr;
    ...
    DMACurrentTxDesc->Buffer1Addr = DMACurrentPTPTxDesc ->Buffer1Addr ;
    DMACurrentTxDesc->Buffer2NextDescAddr = DMACurrentPTPTxDesc ->Buffer2NextDescAddr;
...
uint32_t ETH_HandlePTPRxPkt(uint8_t *pbuf, uint32_t *PTPRxTab)
{
    ...
    *PTPRxTab = DMACurrentRxDesc->Buffer2NextDescAddr;
    ...
    DMACurrentRxDesc->Buffer1Addr = DMACurrentPTPRxDesc ->Buffer1Addr ;
    DMACurrentRxDesc->Buffer2NextDescAddr = DMACurrentPTPRxDesc ->Buffer2NextDescAddr;
