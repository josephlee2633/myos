...
#define CLKCTLR_CLEAR_MASK        ((uint32_t)0x7FFF8100)
/* POWER_PWRSTATE mask */
...
void SDIO_Init(SDIO_InitPara* SDIO_InitParaStruct)
{
    ...
    /* Configure the SDIO_CLKCTLR_DIVH bit */
    if(SDIO_InitParaStruct->SDIO_ClockDiv > 255)
    {
        temp |= SDIO_CLKCTLR_DIVH;
        SDIO_InitParaStruct->SDIO_ClockDiv -= 256;
    }
