...
{
    uint32_t divider = 0, apbclock = 0, temp = 0,fractionaldivider=0;
    ...
    divider = (uint32_t)((25*apbclock) / (4*(USART_InitParaStruct->USART_BRR)));

    temp= (divider / 100) << 4;

      /* Determine the fractional part */
    fractionaldivider = divider - (100 * (temp >> 4));

    temp |= ((((fractionaldivider * 16) + 50) / 100)) & ((uint8_t)0x0F);
     
    USARTx->BRR = (uint16_t)temp;
}
...
/**
  ...
  *     @arg USART3
  *     @arg USART6
  * @param  USART_ClockInitStruct: pointer to a USART_ClockInitPara structure that contains 
